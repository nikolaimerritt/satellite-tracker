...
export class SatelliteFetcherService {
    ...
    //         } as EcfVec3<number>;
    //     };
    //     const squaredDistance = (a: EciVec3<number>, b: EciVec3<number>) =>
    //         Math.sqrt(squaredNorm(subtract(a, b)));
    ...
    //                 normalize(
    //                     throwIfUndefined(
    //                         this.cartesianCoordsAtTime(
    //                             trajectory,
    //                             new Date(time),
    //                         ),
    //                     ),
    //                 );
    //             const squaredDistanceFromObserver = (time: number) =>
    //                 squaredDistance(path(time), normalizedObserver);

    //             const squaredDistanceSamples = [...Array(samples).keys()].map(
    //                 (i) => {
    //                     const time =
    //                         startTime.getTime() +
    //                         ((endTime.getTime() - startTime.getTime()) * i) /
    //                             samples;
    //                     return [
    //                         (time - startTime.getTime()) / 1000_000,
    //                         squaredDistanceFromObserver(time),
    //                     ];
    //                 },
    //             );
    //             console.log(
    //                 'squared distance',
    //                 JSON.stringify(squaredDistanceSamples),
    //             );
    ...
    //                 ) {
    //                     const t =
    //                         ((left[0] + right[0]) / 2) * 1000_000 +
    //                         startTime.getTime();
    //                     const s = squaredDistanceFromObserver(t);
    //                     inflectionSamples.push([t, s]);
    //                 }
    //             }
    //             console.log('inflections', JSON.stringify(inflectionSamples)); // also works

    //             const closestPoint = [...inflectionSamples].sort(
